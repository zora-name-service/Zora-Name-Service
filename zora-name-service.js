...
const zoraNS=class{constructor(Provider){this.Provider=Provider;
...
const c_address="0xBEcA65B33Fc81a46F8c3655183EcAD4E5447B9a7";this.contract=new Provider.eth.Contract(abi,c_address);}
...
setResolveAddress(name,new_address,account,gasAmount){
	...
	//return this.contract.methods.setAddress(name,new_address).send({gasLimit: String(gasAmount),gasPrice: String(gasPrice),from:account}).once("error",(err)=>{
		console.log(err);
		}).then((receipt)=>{
			return receipt;
		}).catch(function(error){
...
setData(name,area,value,account,gasAmount){
	...
	//return this.contract.methods.setDataAddress(name,area,value).send({gasLimit: String(gasAmount),gasPrice: String(gasPrice),from:account}).once("error",(err)=>{
		console.log(err);
		}).then((receipt)=>{
		return receipt;
		}).catch(function(error){
...
setPrimaryAddress(name,account,gasAmount){
	...
	//return this.contract.methods.setPrimaryAddress(name).send({gasLimit: String(gasAmount),gasPrice: String(gasPrice),from:account}).once("error",(err)=>{
		console.log(err);
		})
		.then((receipt)=>{
			return receipt;
...
Register(name,ref_address,account,value,gasAmount) {
  ...
  //return this.contract.methods.Register(name.replace(".scroll","")).send({gasLimit: String(gasAmount),gasPrice: String(gasPrice),from: account,value: value}).once("error", (err) => {
      console.log(err);
    })
    .then((receipt) => {
    return receipt;
